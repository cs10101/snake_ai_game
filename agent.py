...
device = torch.device("cpu")   # Apple CPU
...
class Agent:
    ...
    def __init__(self):
        ...
        self.gamma = 0.9 # discount rate, this number must always be smaller than 1
...
def train(num_games = None):
    ...
    while agent.n_games < max_games:
        ...
        if done:
            # train the loing memory, plot results
            game.reset()
            agent.n_games += 1
...
if __name__ == '__main__':
    ...
    # if the user provides a number in the terminal, use it
    if len(sys.argv) > 1:
        try:
        ...
        print("cannot train for negative games, running forever now. \nAlways enter a positive whole number")
